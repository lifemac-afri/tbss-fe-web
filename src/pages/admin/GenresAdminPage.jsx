...
export default function GenresAdminPage() {
  ...
  const loadSubGenres = (genreId) => {
    ...
    get(`/api/admin/sub-genres/?genre=${genreId}&page_size=500`).then(data => {
      setSubGenres(Array.isArray(data) ? data : (data.results || []));
  ...
  const handleSaveGenre = async () => {
    ...
    try {
      if (editingGenre) {
        const updated = await put(`/api/admin/genres/${editingGenre.id}/`, { name: genreForm.name });
        setGenres(prev => prev.map(g => g.id === updated.id ? updated : g));
        if (selectedGenre?.id === updated.id) setSelectedGenre(updated);
      } else {
        const created = await post('/api/admin/genres/', { name: genreForm.name });
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Genres & Sub-genres</h1>
          <p className="text-sm text-gray-500 mt-1">Click a genre to manage its sub-genres</p>
        </div>
        <button onClick={fetchGenres} disabled={loadingGenres} className="p-2 rounded-xl hover:bg-gray-100 text-gray-400 hover:text-gray-700 transition-colors disabled:opacity-30" title="Refresh">
