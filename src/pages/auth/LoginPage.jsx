...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      ...
      const destination = result.user?.is_staff ? '/admin' : returnTo;
      navigate(destination, { replace: true });
    } else {
      const newAttempts = failedAttempts + 1;
      setFailedAttempts(newAttempts);
      if (newAttempts >= 5) {
        setApiError('Too many failed attempts. Please wait 15 minutes before trying again.');
      } else if (result.code === 'UNVERIFIED') {
        setApiError('Please verify your email before signing in. Check your inbox for the verification link.');
      } else {
    ...
    <div className="w-full max-w-md">
      <div className="bg-white rounded-2xl shadow-card px-8 pt-8 pb-10 border border-gray-100">
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-gray-500 mt-1">Sign in to your TBSS account</p>
        </div>
        ...
        {apiError && (
          <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg text-sm text-red-600">
            ...
            {apiError.includes('verify your email') && (
              <Link
                to="/verify-email"
                className="block mt-1.5 text-[#F46B03] font-medium hover:underline"
              >
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          <div className="flex justify-end mb-4 mt-1">
            <Link to="/forgot-password" className="text-xs text-[#F46B03] hover:underline font-medium">
              Forgot password?
            </Link>
          </div>
        ...
        <p className="text-center text-sm text-gray-500 mt-5">
          ...
          <Link to="/register" className="text-[#F46B03] font-semibold hover:underline">
            Create one
          </Link>
        </p>
      </div>
